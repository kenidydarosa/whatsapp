...
const GroupMembersDialog = () => {
	return (
		<Dialog>
			<DialogTrigger>
				<p className='text-xs text-muted-foreground text-left'>Visualizar membros</p>
			</DialogTrigger>
			<DialogContent>
				<DialogHeader>
					<DialogTitle className='my-2'>Membros atuais</DialogTitle>
					<DialogDescription>
								...
								<div key={user._id} className={`flex gap-3 items-center p-2 rounded`}>
									...
									<div className='w-full '>
										<div className='flex items-center gap-2'>
											...
											{user.admin && <Crown size={16} className='text-yellow-600' />}
										</div>
									</div>
								</div>
							))}
